...
}// так а что тебе надо вщять, возьми
...
void gnomesort(Orders* order) {
	...
	while (i < amO) {
		...
		if (order[i - 1].Count <= order[i].Count) {
			i++;
		}
		else {
	...
	}// ТАК ЭТА ФУНКЦИЯ НЕ ПОДЧЁРКИВАЕТ ЖЕ
	for (int i = 0; i < amO; i++)
	{
		cout << order[i].Count << "\n";
	}
...
void InsertionSort(Orders* order) {
	for (int i = 1; i < amO; ++i) {
		int x = order[i].Count;
		int j = i;
		while (j > 0 && order[j - 1].Count > x) {
			order[j].Count = order[j - 1].Count;
			--j;
		}
		order[j].Count = x;
	}
...
void ShakerSort(Orders* order) {
	...
	while (left <= right) {
		...
		for (int i = left; i < right; ++i) {
			if (order[i].Count > order[i + 1].Count) {
				swap(order[i], order[i + 1]);
			}
		}
		--right;
