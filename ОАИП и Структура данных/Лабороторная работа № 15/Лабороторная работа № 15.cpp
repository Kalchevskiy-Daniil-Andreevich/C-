//Быстрая сортировка была разработана Хоаром и представляет собой рекурсивный алгоритм. Массив делится на две части относительно некоторого значения, называемого медианой.
//В ле-вой части числа меньше медианы, в правой – больше. 
//Эти части сортируются независимо, для чего вызывается та же самая функция.Процесс про - должается до тех пор, пока очередная часть массива не станет содержать один элемент.
#include<iostream>
#define N 10
using namespace std;
